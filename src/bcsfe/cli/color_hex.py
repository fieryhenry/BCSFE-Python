# taken from https://gitlab.com/dslackw/colored/-/blob/1.4.4/colored/hex.py
...
def hex_to_ansi(color: str):
    ...
    for xterm_color, hexcolor in _xterm_colors.items():
        if hexcolor == color:
            return xterm_color
    ...
    cube: Callable[[int], int] = lambda x: x * x

    f: Callable[[str, int], int] = lambda hex_val, ref: cube(int(hex_val, 16) - ref)

    min_cube_d = cube(0xFFFFFF)
    nearest = "15"

    for k, h in _xterm_colors.items():
